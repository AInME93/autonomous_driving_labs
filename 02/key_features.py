...
def filter_distance(matches):
    ...
    # for m in matches:
    #     if m.distance < 5.0 * matches[0].distance:
...
def feature_match_points(dir_path, files, first_idx, idx_increment):
    ...
    matches_img1 = [kp1[m.queryIdx].pt for m in matches[0:8]]
    matches_img2 = [kp2[m.trainIdx].pt for m in matches[0:8]]
...
def main():
    ...
    idx = 0
    while idx < len(files)-idx_increment and idx != 30:
        try:
